...
def _postprocess_docx(path: Path) -> None:
    ...
    try:
        ...
        print("python-docx not installed; skipping docx post-processing.")
        return
    ...
    # Get style IDs by name for later use.
    name_to_id = {s.name: s.style_id for s in doc.styles}

    def make_caption_para(label: str, seq_name: str, text: str):
        """Build a paragraph element with: '<label> { SEQ <seq_name> \\* ARABIC } <text>'."""
        p = OxmlElement("w:p")
        pPr = OxmlElement("w:pPr")
        pStyle = OxmlElement("w:pStyle")
        pStyle.set(qn("w:val"), CAPTION_STYLE)
        pPr.append(pStyle)
        p.append(pPr)

        def add_run(t: str, preserve: bool = False):
            r = OxmlElement("w:r")
            tn = OxmlElement("w:t")
            if preserve:
    ...
    for tbl in body.findall(qn("w:tbl")):
        ...
        if existing_cap is not None:
            ...
            text = "".join(
                (t.text or "") for t in existing_cap.findall(".//" + qn("w:t"))
            ).strip()
            new_cap = make_caption_para("表", "Table", text)
            existing_cap.addprevious(new_cap)
            body.remove(existing_cap)
        else:
            tbl.addprevious(make_caption_para("表", "Table", ""))
    ...
    for p in list(body.findall(qn("w:p"))):
        ...
        if ps is None or ps.get(qn("w:val")) not in {
            "CaptionedFigure", "Captioned Figure"
        }:
            continue
        descr = ""
        docpr = p.find(".//{%s}docPr" % WP_NS)
        if docpr is not None:
            descr = (docpr.get("descr") or docpr.get("title") or "").strip()
        cap = make_caption_para("图", "Figure", descr)
        p.addnext(cap)
    ...
    # 6) Bibliography: pandoc/citeproc emits each reference entry as a
    #    "Normal"-styled paragraph at the #refs placeholder. Locate the
    #    "参 考 文 献" heading and restyle every following paragraph (until
    #    the next level-1 heading, or end of body) to "列表段落（无编号）".
    BIB_PARA_STYLE = "列表段落（无编号）"
    bib_target_id = None
    for s in doc.styles:
        if s.name == BIB_PARA_STYLE:
            bib_target_id = s.style_id
            break
    if bib_target_id:
        name_to_id = {s.name: s.style_id for s in doc.styles}
        heading1_ids = {
        ...
        for i, p in enumerate(body_ps):
            ps = p.find(qn("w:pPr") + "/" + qn("w:pStyle"))
            if ps is None or ps.get(qn("w:val")) not in heading1_ids:
                continue
            txt = "".join(
        ...
        if ref_idx is not None:
            for p in body_ps[ref_idx + 1:]:
                ps = p.find(qn("w:pPr") + "/" + qn("w:pStyle"))
                sval = ps.get(qn("w:val")) if ps is not None else None
                if sval in heading1_ids:
                ...
                if ps is None:
                    ...
                    pPr.insert(0, ps)
                ps.set(qn("w:val"), bib_target_id)

    # 6c) Figure/table notes: any paragraph whose text starts with "注：" /
    #     "注:" / "Note:" / "Note：" gets the 图表注 paragraph style.
    note_style_id = None
    for s in doc.styles:
        if s.name == "图表注":
            note_style_id = s.style_id
            break
    if note_style_id:
        for p in body.findall(qn("w:p")):
            ...
            if not (
                txt.startswith("注：") or txt.startswith("注:")
                or txt.startswith("Note:") or txt.startswith("Note：")
            ):
                continue
            # Tables can also hold paragraphs in cells; iterate body+cells.
            pPr = p.find(qn("w:pPr"))
            if pPr is None:
                pPr = OxmlElement("w:pPr")
                p.insert(0, pPr)
            for old in pPr.findall(qn("w:pStyle")):
                pPr.remove(old)
            pStyle = OxmlElement("w:pStyle")
            pStyle.set(qn("w:val"), note_style_id)
            pPr.insert(0, pStyle)
        # Also handle paragraphs inside table cells.
        for p in body.findall(".//" + qn("w:tbl") + "//" + qn("w:p")):
            txt = "".join(
                (t.text or "") for t in p.findall(".//" + qn("w:t"))
            ).lstrip()
            if not (
    ...
    # 6d) Move pandoc's auto-generated TOC to between the abstract and the
    #     body (before the RmdMarker3 paragraph) and rename it 目录. MUST run
    #     before §1 injection so the SDT (sitting before RmdMarker2 in the raw
    #     pandoc output) survives the deletion in step 6e.
    _relocate_toc(doc)

    # 6e) Inject template §1 verbatim (cover + English cover + originality &
    ...
    # 7) Per-Rmd section breaks: each chapter gets the header/footer of the
    #    template's Nth section, where N is the leading non-zero digit of the
    #    Rmd filename prefix (020 → §2, 030 → §3, ...). RmdMarkerN markers
    #    inserted in merge_sources flag chapter ownership.
    if template_doc.is_file():
        _apply_per_rmd_sections(doc, template_doc)

    # 8) Ensure a page break before every level-1 heading (每个 # 之前分页).
    ...
    # 9) Equations: number display math (单行公式自动编号).
    #    A paragraph is considered display math if it contains m:oMath and no other text.
    #    We apply the '公式' style and append a tab + (SEQ Equation).
    M_NS = "http://schemas.openxmlformats.org/officeDocument/2006/math"
    for p in body.findall(qn("w:p")):
        # Check for oMath (Office Math).
        maths = p.findall(".//" + qn("m:oMath"))
        if not maths:
            continue

        # Check if it's "math-only" (no non-whitespace text runs).
        text = "".join((t.text or "") for t in p.findall(".//" + qn("w:t"))).strip()
        if text:
            continue
        ...
        if "公式" in name_to_id:
            ...
            for old in pPr.findall(qn("w:pStyle")):
                pPr.remove(old)
            pStyle = OxmlElement("w:pStyle")
            pStyle.set(qn("w:val"), name_to_id["公式"])
            pPr.insert(0, pStyle)

        # Append a tab and the (SEQ Equation) field.
        # We ensure two tabs: one at the start (to center math) and one before numbering (to right-align).
        # We also force explicit tab stops to ensure 'far right' alignment.
        pPr = p.find(qn("w:pPr"))
        if pPr is None:
            pPr = OxmlElement("w:pPr")
            p.insert(0, pPr)
        
        # 1) Set paragraph alignment to left (so tabs work).
        jc = pPr.find(qn("w:jc"))
        if jc is not None:
            pPr.remove(jc)
        jc = OxmlElement("w:jc")
        ...
        # 2) Set explicit tab stops: Center at ~8cm (4536 twips), Right at ~16cm (9072 twips).
        ...
        t1.set(qn("w:pos"), "4536")
        tabs.append(t1)
        
        t2 = OxmlElement("w:tab")
        t2.set(qn("w:val"), "right")
        t2.set(qn("w:pos"), "9072")
        tabs.append(t2)

        # 3) Insert a tab at the very beginning of the paragraph to center the math.
        r_tab_start = OxmlElement("w:r")
        r_tab_start.append(OxmlElement("w:tab"))
        # Insert after pPr (index 0)
        p.insert(1, r_tab_start)

        # 4) Append a tab before the numbering to push it to the far right.
        r_tab_end = OxmlElement("w:r")
        r_tab_end.append(OxmlElement("w:tab"))
        p.append(r_tab_end)

        def add_run(p_el, t_text):
            r = OxmlElement("w:r")
            t = OxmlElement("w:t")
            t.text = t_text
        ...
        instr = OxmlElement("w:instrText")
        instr.set(qn("xml:space"), "preserve")
        instr.text = " SEQ Equation \\* ARABIC "
        f2.append(instr)
        p.append(f2)
    ...
    # 10) Keywords: paragraphs starting with "关键词" or "keywords" get the "关键词" style.
    kw_style_id = name_to_id.get("关键词")
    if kw_style_id:
        for p in body.findall(qn("w:p")):
            txt = "".join((t.text or "") for t in p.findall(".//" + qn("w:t"))).lstrip()
            low = txt.lower()
            if not (
                low.startswith("关键词：") or low.startswith("关键词:")
                or low.startswith("keywords:") or low.startswith("keywords：")
            ):
                continue
            ...
            for old in pPr.findall(qn("w:pStyle")):
                pPr.remove(old)
            pStyle = OxmlElement("w:pStyle")
            pStyle.set(qn("w:val"), kw_style_id)
            pPr.insert(0, pStyle)

    # 10) settings.xml: set <w:updateFields w:val="true"/> so Word refreshes
...
def _inject_template_section1(doc, template_path: Path, replacements) -> None:
    ...
    for c in list(body):
        if c.tag != qn("w:p"):
            continue
        ps = c.find(qn("w:pPr") + "/" + qn("w:pStyle"))
        if ps is None:
            continue
        if (ps.get(qn("w:val")) or "").startswith("RmdMarker"):
            first_marker = c
            break
...
def _ensure_page_break_before_headings(doc) -> None:
    ...
    heading_ids = {
        ...
        name_to_id.get("非编号章节标题"),
        name_to_id.get("非编号章节标题（目录不显示）"),
        name_to_id.get("ABSTRACT"),
    }
    heading_ids.discard(None)
    ...
    for p in body.findall(qn("w:p")):
        ...
        if ps is None:
            continue
        if ps.get(qn("w:val")) in heading_ids:
            headings.append(p)
...
def _relocate_toc(doc) -> None:
    ...
    # Build a page-break paragraph (empty paragraph holding only w:br type=page).
    page_break_p = OxmlElement("w:p")
    pb_r = OxmlElement("w:r")
    pb_br = OxmlElement("w:br")
    pb_br.set(qn("w:type"), "page")
...
def _apply_per_rmd_sections(doc, template_path: Path) -> None:
    ...
    body = doc.element.body
    # Resolve target heading style IDs by display name (the docx's internal
    # style IDs may be auto-generated like "af4"/"afb").
    name_to_id = {s.name: s.style_id for s in doc.styles}
    ...
    markers_to_remove = []
    current_digit = None
    marker_pat = _re.compile(r"^RmdMarker(\d+)$")
    ...
    for p in list(body.findall(qn("w:p"))):
        ...
        if sval in HEADING_STYLES and current_digit is not None:
            ...
            if heading_text.upper() == "ABSTRACT" and ABSTRACT_ID:
                target = ABSTRACT_ID
            if target and ps is not None and ps.get(qn("w:val")) != target:
    ...
    def build_sectpr_for_digit(d):
        # Clone the template's i-th sectPr so pgMar, pgSz, pgNumType, cols,
        # vAlign, etc. all come from the matching template section. Then
        # remap header/footer r:id attributes onto the output's rIds, and
        # drop any reference whose target file is missing in the output.
        ...
        for child in list(new):
            if child.tag not in (
                ...
                continue
            tpl_rid = child.get(f"{{{R_NS}}}id")
            target = tpl_rid_to_target.get(tpl_rid)
            out_rid = out_target_to_rid.get(target) if target else None
            if out_rid:
                child.set(f"{{{R_NS}}}id", out_rid)
            else:
                new.remove(child)
        return new
    ...
    for i in range(1, len(chapters)):
        ...
        if current_digit == prev_digit:
            ...
            if p_chap.getparent().tag != qn("w:sdtContent"):
                ...
                pb_r.append(pb_br)
                page_break_p.append(pb_r)
                p_chap.addprevious(page_break_p)
            continue

        prev = p_chap.getprevious()
        # Walk back over non-paragraph siblings AND marker paragraphs
        # (the latter get deleted at the end and would lose the sectPr).
